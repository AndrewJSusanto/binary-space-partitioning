...
typedef struct BSPTree{
	...
	void* (*GetSubtreeRoot)(ListPtr data); // look at SelectPartitionRoot, this should find your element that you will use to partition a list
	ListPtr (*GetSubtrees)(void* parent, ListPtr remaining); // pass in GetPartitions; ListPtr returned is a list of left children, 
	int (*DataCompare)(void* data1, void* data2); // pass as SegmentsEqual. compares two segments together, 0 if equal, something else if not (you will be passing this to a list object)
	void (*DataPrinter)(void* data); // pass as PrintSegment. function which printsa  line segment (you will be passing this to a list object)
	void (*FreeData)(void* data); // pass as DestroySegment
} BSPTree;
